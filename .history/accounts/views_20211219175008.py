...
def employeeMonthlyPayrol(request,user_id,empId):
    if request.method == 'POST':
        ...
        while initialDate<endDate:
            empInfo=dict()
            userPhoto=EmployeeImages.objects.filter(empId=empId,saveDateTime__startswith=initialDate).exists()
            if userPhoto:
                empInfo['photoVerified']=user
            else:
                empInfo['photoVerified']="not present"
            userLocation=EmployeeWorkLocations.objects.filter(empId=empId,updated_at__startswith=initialDate).exists()
            if userLocation:
                empInfo['time']="Y"
            else:
                empInfo['time']="not present"
            print(userPhoto.is_verified)
            print('----------------')
            print(userLocation)
            initialDate=datetime.date(initialDate)+timedelta(1)
            print(initialDate)

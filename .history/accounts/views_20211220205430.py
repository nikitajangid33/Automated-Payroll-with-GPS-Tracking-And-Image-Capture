...
def employeeMonthlyPayrol(request,user_id,empId):
    if request.method == 'POST':
        ...
        initialDate=datetime(int(dateArray[0]),int(dateArray[1]),int(dateArray[2]))
         #making format to datetime for end date
        dateArray=endDate.split('-')
        endDate=datetime(int(dateArray[0]),int(dateArray[1]),int(dateArray[2]))
        print(initialDate)
        print(endDate)
        ...
        while initialDate<=endDate:
            ...
            initialDate=datetime(int(dateArray[0]),int(dateArray[1]),int(dateArray[2]))+timedelta(days=1)
            print(datetime.date(initialDate))
            #making same format for end date
            dateArray=endDate.split('-')
            endDate=datetime(int(dateArray[0]),int(dateArray[1]),int(dateArray[2]))

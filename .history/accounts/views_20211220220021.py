...
#tocalculate monthly payroll
def employeeMonthlyPayrol(request,user_id,empId):
    if request.method == 'POST':
        initialDate=request.POST['startTime']
        endDate=request.POST['endTime']
        #making format to datetime for initial date
        dateArray=initialDate.split('-')
        initialDate=date(int(dateArray[0]),int(dateArray[1]),int(dateArray[2]))
         #making format to datetime for end date
        dateArray=endDate.split('-')
        endDate=date(int(dateArray[0]),int(dateArray[1]),int(dateArray[2]))
        print(initialDate)
        print(endDate)
        finalArray=[]
        ...
        while initialDate<=endDate:
            empInfo=dict()
            empInfo['date']=initialDate
            #fetching image to check if that validate or not
            ...
            if userPhoto.exists():
                ...
                empInfo['photoVerified']="Not Present"
            #fetching signoff and signout time
            ...
            print(initialDate)
            finalArray.append(empInfo)

        print(finalArray)
        return render(request,'EmployeePayrol.html',{'user_id':user_id,'empId':empId,'data':finalArray,'currdate':initialDate,'enddate':endDate})
        
    else:
        return render(request,'EmployeePayrol.html',{'user_id':user_id,'empId':empId,'data':[]})
